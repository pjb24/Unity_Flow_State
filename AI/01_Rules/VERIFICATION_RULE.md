...
빠른 연속 조작, 짧은 입력 타이밍, 동일 프레임 또는 인접 프레임의 상태 변화처럼 사람이 안정적으로 재현하고 판정하기 어려운 항목은 수동 검증으로 요구하지 않고 Edit Mode 또는 Play Mode Test로 검증한다.
...
수동 검증 절차는 빠른 조작 속도나 정밀한 입력 타이밍을 요구하지 않는다.

입력 잔류, 중복 실행, 빠른 상태 전환과 짧은 입력 Window는 자동 Test 결과로 판정한다.
