...
Null 확인을 위해 nullable `?`, null 조건부 `?.` 또는 null 병합 `??` 문법을 새로 사용하지 않는다.

Null 가능 참조는 명시적인 `if` 조건으로 검사한다.

선택적인 값 타입은 nullable 값 타입 대신 존재 여부를 나타내는 Bool과 값 Property를 분리하여 표현한다.

기존 코드에서 해당 문법을 사용하는 경우 현재 작업과 직접 관련된 수정 범위에서만 규칙에 맞게 변경하며 무관한 전체 리팩토링을 수행하지 않는다.
