...
수집한 플레이 입력 상태를 필요한 System에 제공한다.

플레이 입력 장치와 플레이 관련 System 사이의 연결을 담당한다.
...
Player Action Map은 플레이 중 입력을 처리하기 위한 입력 집합이다.

PlayerInputSystem은 Player Action Map의 활성 상태를 관리한다.
...
- 플레이 중 입력만 관리한다.
- UI Action Map을 관리하지 않는다.
- 어떤 Action Map을 사용할지는 결정하지 않는다.
- GameSystem의 요청에 따라서만 Player Action Map의 상태를 변경한다.
- UI 입력을 관리하지 않는다.
- 입력의 의미를 판단하지 않는다.
